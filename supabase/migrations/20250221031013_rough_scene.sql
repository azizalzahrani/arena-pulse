/*
  # Analytics Schema and Policies

  1. Schema
    - Creates analytics schema for real-time data
    
  2. Tables
    - real_time_metrics: Stores zone-specific metrics
    - zone_config: Stores zone configurations and thresholds
    - alerts: Stores system alerts and notifications
    - traffic_flow: Stores zone traffic patterns
    
  3. Security
    - Enables RLS on all tables
    - Creates appropriate access policies with existence checks
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'real_time_metrics' 
    AND indexname = 'idx_real_time_metrics_zone_timestamp'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    AND indexname = 'idx_alerts_zone_status'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE schemaname = 'analytics' 
    AND tablename = 'traffic_flow' 
    AND indexname = 'idx_traffic_flow_zones'
  ) THEN
...
DO $$
BEGIN
  DROP POLICY IF EXISTS "Allow read access to real-time metrics" ON analytics.real_time_metrics;
  DROP POLICY IF EXISTS "Allow read access to zone config" ON analytics.zone_config;
  DROP POLICY IF EXISTS "Allow admins to manage zone config" ON analytics.zone_config;
  DROP POLICY IF EXISTS "Allow read access to alerts" ON analytics.alerts;
  DROP POLICY IF EXISTS "Allow admins to manage alerts" ON analytics.alerts;
  DROP POLICY IF EXISTS "Allow read access to traffic flow" ON analytics.traffic_flow;
END $$;
